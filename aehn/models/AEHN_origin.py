...
class AEHN_origin(BaseModel):
    """
    AEHN that delta using lambda and hidden state to calculate.
    """
    ...
    def predict(self, sess, batch_data, **kwargs):
        ...
        loss, pred_types, pred_time = sess.run(
            [self.loss, self.pred_types, self.pred_time],
            feed_dict=fd)
    ...
    def intensity_layer(self, x_input, reuse=tf.AUTO_REUSE):
        ...
        with tf.name_scope('intensity_layer'):
            ...
            def move_forward_fn(i, past_lambdas):
                # shape -> [batch_size, i, hidden_dim]
                mu = mus[:, i]
                # shape -> [batch_size, i, hidden_dim]
                alpha = alphas[:, i, :i]
                # shape -> [batch_size, i, 1]
                target_elapse = target_elapses[:, i, :i]

                # (batch_size, i, hidden_dim)
                stacked_past_lambdas = past_lambdas[:, :i]
                tiled_current_state = tf.tile(x_input[:, i: i + 1], [1, i, 1])

                # (batch_size, i, hidden_dim + hidden_dim)
                delta_input = tf.concat([stacked_past_lambdas, tiled_current_state], axis=-1)
                # (batch_size, i, 1)
                delta = delta_layer(delta_input)

                cur_lambda = self.compute_latent_lambda(mu, alpha, delta, target_elapse)
                past_lambdas = tf.concat([past_lambdas,
                                          tf.expand_dims(cur_lambda, axis=1)], axis=1)

                return i + 1, past_lambdas

            # shape -> [batch_size, 1, hidden_dim]
            lambdas = create_tensor([batch_size, 1, self.hidden_dim], 0.0)
            # shape -> [batch_size, max_len, hidden_dim]
            _, lambdas = tf.while_loop(lambda i, _: tf.less(i, max_len),
                                       move_forward_fn,
                                       [1, lambdas],
                                       [tf.constant(1).get_shape(),
            ...
            right = tf.tile(lambdas[:, None, :, :], [1, max_len, 1, 1])
